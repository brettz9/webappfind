...
1. Allow for map between the fundamental functional modes and user-created custom modes (and change mode to be passed as a generic flag value rather than part of the flag name!). For example, filetypes.json might have a map to indicate that the custom mode, "viewsource" was equivalent in function to "view", the custom mode, "viewoutput" was also equivalent to "view", and the custom mode, "editoutput" was equivalent to "edit", etc.; filetypes.json could then indicate separate default web app handlers for "view", "viewsource", or "viewoutput", "edit", etc. (or sites could register to handle these different modes). This would help the user avoid the need to swap handlers (or modify filetypes.json) whenever they wished to go directly to an app (or a part of an app) which brought the precise functionality they were seeking at the moment, and it would allow niche apps (such as HTML source viewers) to avoid registering themselves as handlers in a manner that would conflict with other normally more useful apps that would act on the same file type (e.g., a WYSIWYG HTML viewer). At the moment, I am thinking that custom modes should probably be namespaced (perhaps simply with the fundamental mode name) in order to prevent conflicts with any future fundamental modes, while fundamental modes should be limited to those which genuinely require a distinct mode of transmission or privileges (e.g., editing vs. viewing or normal vs. binary). Mapping could perhaps be done through the file name in place of filetypes.json, if, for example, WebAppFind would look for a custom mode in the name prior to the extension, e.g., "myfile.viewsource.mytype" would be seen as a derivative of the fundamental "view" type, but with a custom name, "viewsource" (and the file type would, as usual, be detected as "mytype"). (Hyphens or such would be useful for namespacing, but the current HTML spec does not allow protocols to be registered with them present.)
