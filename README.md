...
1. Allow for map between the fundamental functional modes and user-created custom modes. For example, filetypes.json might have a map to indicate that the custom mode, "view-source" was equivalent in function to "view", the custom mode, "view-output" was also equivalent to "view", and the custom mode, "edit-output" was equivalent to "edit", etc. This would help the user to avoid the need to swap handlers (or modify filetypes.json) when they wanted to go directly to an app (or a part of an app) which brought the exact functionality they were seeking. (Custom modes should probably be namespaced to prevent conflicts with any future fundamental modes.) Mapping could perhaps be done through the file name in place of filetypes.json, if, for example, WebAppFind would look for a custom mode in the name prior to the extension, e.g., "myfile.view-source.mytype" would be seen as a derivative of the fundamental "view" type, but with a custom name, "view-source" (and the file type would, as usual, be detected as "mytype").
