...
const registerComponent = require('./commandLineHandlerComponent');
/*
function l (str) {
  console.log(str);
}
...
function registerPermittedPathForURI (uri, mode, customMode, path) {
  ...
  if (!permittedURIDict[uri][mode][customMode].includes(path)) {
    permittedURIDict[uri][mode][customMode].push(path);
  }
}
function isPermittedPathForURI (uri, mode, customMode, path) {
  return permittedURIDict && permittedURIDict[uri] && permittedURIDict[uri][mode] && permittedURIDict[uri][mode][customMode] && permittedURIDict[uri][mode][customMode].includes(path);
}
  ...
  tabs.activeTab.on('ready', function (tab) {
    ...
    function start () {
      worker.port.emit('webappfindStart', {
        uri: tab.url,
        pathID,
        content: mode.includes('binary') ? file.read(path, 'b') : file.read(path)
      }); // , path:path
    }
  ...
  tabs.activeTab.on('ready', function (tab) {
    ...
    function start () {
      worker.port.emit('webappfindDirectoryPath', {
        uri: tab.url,
        path // We might add a list of file names, etc.
      });
    }
...
module.exports = function () { // listener
  ...
  registerComponent({
    ...
    handler (cmdLine) {
      ...
      try {
        ...
        if (!standardModes.includes(mode)) { // Default instead to "view"?
  ...
  return {
    // Public API
    isPermittedPathForURI,
    removePermittedPathForURI
  };
};
