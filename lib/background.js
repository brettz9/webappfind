...
async function populateDynamicCMItems () {
    ...
    try {
        ...
        Object.entries(JSON.parse(commands || '{}')).forEach(([id, commandDetail]) => {
            p = p.then(() => {
...
async function addDynamicCMContent (id, details) {
    ...
    await browser.storage.local.set({[menuItemId]: JSON.stringify(details)});
    browser.contextMenus.create({
        id: menuItemId,
...
browser.contextMenus.onClicked.addListener(async (info, tab) => {
    ...
    if ((/(atyourcommand|user)-/).test(menuItemId)) {
        ...
        if (menuItemId.startsWith('user-')) {
            try {
                const {[menuItemId]: details} = JSON.parse(
                    await browser.storage.local.get(menuItemId) ||
                    '{}'
                );
                const commandlineOutput = await execute(menuItemId, details);
        ...
        const top = 10, left = 100; // Todo: Persist changes
