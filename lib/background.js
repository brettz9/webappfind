...
nativePort.onMessage.addListener(async (parsedObj) => {
    ...
    p.onMessage.addListener(function ({type, pathID, content}) {
        // Todo: We might instead post against *all* tabs in `ports` with the same URL
        switch (type) {
        case 'read':
            // Todo: Get fresh copy
            nativePort.postMessage({pathID});
            p.postMessage({result: parsedObj, type: 'start'});
            break;
        case 'save':
    ...
    if ('file' in parsedObj) {
        ...
        pathFileMap[parsedObj.pathID] = parsedObj.file;
    }
    p.postMessage({result: parsedObj, type: 'start'});
});
