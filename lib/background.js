...
async function addDynamicCMContent (id, details) {
    ...
    browser.contextMenus.create({
        ...
        contexts: ['page', 'selection'] // User's own context applied on top of this
        // icons: ''
    });
    ...
    //   further
    if (!details.textOnly &&
        (details.ownContext || details.restrictContexts)) {
        setSelectorContext({
            menuItemId,
...
/**
* Listen for messages from the webappfind native messaging app (which
* listens for websocket connections, ala "Open"/"Open with...").
*/
nativePort.onMessage.addListener(async (parsedObj) => {
    ...
    const {method, nodeEval, saveEnd, result, pathID, nodeJSON} = parsedObj;
    switch (method) {
    case 'execbuildopen': {
    ...
    case 'readdir': {
        ...
        if (!csPort) {
            console.log('`readdir` message cannot find content script port', csPorts);
            return;
        }
        csPort.postMessage(
            {type: method, result, pathID, error: !!parsedObj.error}
        );
        return;
    }
    }
    ...
    csPort.onMessage.addListener(function ({type, pathID, content}) {
        ...
        case 'unload': // Avoid memory leaks
            ...
            break;
        case 'read': case 'save': case 'readdir':
