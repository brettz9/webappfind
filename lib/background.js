...
browser.contextMenus.onClicked.addListener(async (info, tab) => {
    ...
    if (menuItemId.startsWith('atyourcommand')) {
        ...
        try {
            ...
            window.tabData = {
                itemType, modifiers, checked, wasChecked,
                linkText, linkUrl,
                frameUrl, srcUrl, mediaType,
                favIconUrl,
                selectionText, pageUrl, title, // Getting these on pageData anyways
                pageData: results[0]
            };
        } catch (err) {
    ...
    if (!results || results[0] !== true) {
        // Todo: The second item would ideally just import the first
        //          (waiting on ES6 Modules)
        await browser.tabs.executeScript({
            allFrames: true,
            file: '/utils/i18n.js', // Cross-browser to use absolute path
