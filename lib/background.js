/* eslint-disable node/no-unsupported-features/es-syntax,
  ...
  node/no-unsupported-features/es-builtins, node/no-unsupported-features/node-builtins */
...
window.closeAYC = function (winID) {
  const win = windowsMap.get(parseInt(winID));
...
function flatten (arr) {
  // eslint-disable-next-line array-func/prefer-flat
  return [].concat(...arr);
}
...
browser.webNavigation.onDOMContentLoaded.addListener(async ({tabId, url}) => {
  ...
  `,
    ...
    console.log('err', tabId, err.toString());
  }
}); // , {url: []});

async function addAsYouWish (tabId) {
  const results = await browser.tabs.executeScript(tabId, {
    code: "typeof AsYouWish === 'boolean';"
  });
...
browser.webRequest.onHeadersReceived.addListener( // https://developer.mozilla.org/en-US/Add-ons/WebExtensions/API/webRequest/onHeadersReceived
  ...
  async (details) => {
    ...
    // Store headers
    urlToHeaders.set(tabId, details);

    if (statusCode < 200 || statusCode >= 300) { // Only valid responses (e.g., avoid redirects)
      return;
    }

    // AsYouWish allowed sites
    ...
    // console.log('allowedSites', allowedSites, currentSite);
    if (allowedSites && allowedSites.includes(currentSite)) {
      // Inject
      // console.log('injecting allowed', tabId, JSON.stringify(details), details);
      await addAsYouWish(tabId);
    }
  },
  {
...
browser.contextMenus.onClicked.addListener(async (info, tab) => {
  ...
  if ((/(atyourcommand|user)-/).test(menuItemId)) {
    ...
    // Todo: const results = getPageData({targetElementId, customProperties: []});
    const results = tab.url.startsWith('about:')
      ? [{
        contentType: '',
        pageURL: '',
        pageTitle: '',
      ...
      : await (async () => {
        ...
        return browser.tabs.executeScript(tab.id, {
          allFrames: true,
          code: `getPageData({targetElementId: ${targetElementId}});`,
          runAt: 'document_end'
        });
      })();
    ...
    case 'commands': case 'one-off':
      ...
      break;
    default:
      throw new TypeError('Unknown itemType: ' + itemType);
    }
...
browser.runtime.onConnect.addListener(function (p) {
  if (p.name === 'webappfind-eval-content-script') {
    p.onMessage.addListener(async function ({string, i, method}) {
      switch (method) {
      default:
        throw new TypeError('Unexpected method type: ' + method);
      case 'addonEval': {
        let result;
        try {
...
nativePort.onMessage.addListener(async (parsedObj) => {
  ...
  if (saveEnd) {
    ...
    csPort.postMessage(
      {type: 'saveEnd', pathID: parsedObj.pathID, error: Boolean(parsedObj.error)}
    );
    return;
  }
  // ['file', 'mode', 'site', 'args'];
  ...
  csPort.onMessage.addListener(function ({type, pathID, content}) {
    ...
    switch (type) {
    default:
      throw new TypeError('Unxpected type: ' + type);
