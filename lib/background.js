...
function getPredefinedDirectories () { // eslint-disable-line no-unused-vars
    return predefinedDirectories;
}

function getBrowserProfiles () { // eslint-disable-line no-unused-vars
    return browserProfiles;
}

let predefinedDirectories;
let browserProfiles;
...
browser.webNavigation.onDOMContentLoaded.addListener(async ({tabId}) => {
    try {
        const metas = flattenDepthOne(await browser.tabs.executeScript(tabId, {
            allFrames: true,
            code: `
...
nativePort.onMessage.addListener(async (parsedObj) => {
    ...
    const {directories, profiles, nodeEval, saveEnd} = parsedObj;
    if (directories) {
        predefinedDirectories = directories;
        return;
    }
    if (profiles) {
