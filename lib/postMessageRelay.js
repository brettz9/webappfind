...
window.addEventListener('message', function ({origin, data}) { // Listen for subsequent requests to read the file (necessary also for cases where the 'webapp-view' listener cannot be added immediately, e.g., within our SVG-edit extension which is loaded by the main page doing a script tag injection)
    if (origin !== location.origin ||
        !Array.isArray(data) || data[0] !== 'webapp-read'
    ) {
        return;
    }
...
function webappfindStart (result) {
    if (!addedMessageListener) {
        window.addEventListener('message', function ({origin, data}) {
            if (origin !== location.origin ||
                !Array.isArray(data) || data[0] !== 'webapp-save') {
                return;
            }
            const [, pathID, newContents] = data;
