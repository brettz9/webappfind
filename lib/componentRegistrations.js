...
function startURIWithPermittedPath (uri, method, path, fileType, isConfirmedWebLocalProtocol) {
    ...
    tabs.activeTab.on('ready', function (tab) {
        pageMod.PageMod({
            include: tab.url, // We don't use "uri" because we need to see to what URL any protocol may resolve
            contentScriptFile: data.url('postMessageRelay.js'),
            // contentScriptWhen: 'ready', // Todo: try this instead of default "end"
            contentScriptWhen: 'start', // already within "ready" mode
            ...
            onAttach: function onAttach(worker) {
                ...
                if (method === 'edit' || method === 'binaryedit') {
                    worker.port.on('webappfindSave', function (data) {
                        var pathID = data[0],
                            newContents = data[1],
                            ws = method === 'binaryedit' ? file.open(path, 'wb') : file.open(path, 'w');
...
module.exports = function (listener) {
    ...
    var methods = ['view', 'edit', 'binaryview', 'binaryedit'];
