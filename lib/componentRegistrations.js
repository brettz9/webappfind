...
function startURIWithPermittedPath (uri, method, path, fileType, isConfirmedWebLocalProtocol) {
    ...
    pageMod.PageMod({
        ...
        onAttach: function onAttach(worker) {
            ...
            worker.port.emit('webappfindStart', {uri: uri, pathID: pathID, content: file.read(path)}); // , path:path
        }
    });
            ...
            handler: function clh_handle(cmdLine) {
                ...
                if (path) {
                    ...
                    if (file.exists(fileTypesFile)) {
                        fileTypesJSON = file.read(fileTypesFile); // Todo: Make provisions for optional binary response?
                        try {
